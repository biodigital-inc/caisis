<%@ Control Language="c#" Inherits="Caisis.UI.Modules.Prostate.Eforms.ProstateHPIWithDiagnosisAndComments" CodeFile="ProstateHPIWithDiagnosisAndComments.ascx.cs" CodeFileBaseClass="Caisis.UI.Core.Classes.BaseEFormControl" %>
<%@ Register TagPrefix="euc" Namespace="Caisis.UI.Core.Classes.CustomControls" %>

<script type="text/javascript">
    function setDxBiopsyTotalCores() {
    
        var PathPosCores = $('<% = PathPosCores.ClientID %>');
        var DiagnosisNegativeCores = $('<% = DiagnosisNegativeCores.ClientID %>');
        var PathNumCores = $('<% = PathNumCores.ClientID %>');

        if (isNaN(PathPosCores.value) || isNaN(DiagnosisNegativeCores.value)) PathNumCores.value = '';
        else if (Number(PathPosCores.value) + Number(DiagnosisNegativeCores.value) > 0)  PathNumCores.value = (Number(PathPosCores.value) + Number(DiagnosisNegativeCores.value));
...
<a name="ProstateHPIWithDiagnosisAndComments" /><asp:Label ID="ComponentTitle" runat="server" CssClass="controlBoxTitle" Text="HPI"></asp:Label><br/>
...
<table border="0" cellspacing="0" cellpadding="16" width="600" id="HPI_AddedTextTable">
  
  <tr>
    <td class="eformLargePopulatedTable"><span class="controlBoxTitle" >HPI Comments</span><br/><euc:EformTextArea TextMode="MultiLine" TABLE="NoTable" FIELD="HPI_AddedText" Rows="4"  style="width: 560px;"  Runat="server" ID="HPI_AddedText"/></td>
  </tr>
</table>
...
<table border="0" cellspacing="0" id="NewDiagnosisTable" runat="server"  class="ProstateDiagnosisTable eformLargePopulatedTable">
  ...
  <tr>
    <td >Biopsy Date 
        <euc:EformTextBox Runat="server" ID="DiagnosisDateText" TABLE="Procedures" FIELD="ProcDateText" RecordId="30" style="width:80px;"  ShowCalendar="True" CalcDate="True"   />
	    <euc:EformHidden Runat="server" ID="DiagnosisDate" Table="Procedures" Field="ProcDate" RecordId="30" />    </td>
    <td>BxGG1
  ...
  <tr><td style="border: none;">Biopsy Cores</td><td>Positive<euc:EformTextBox style="width:30px;" TABLE="ProstateBiopsyPath" RecordId="1" ParentRecordId="1" FIELD="PathPosCores" Runat="server" ID="PathPosCores" onchange="setDxBiopsyTotalCores();" />
  <span style="margin-left: 16px;">Negative<euc:EformTextBox style="width:30px;" TABLE="NoTable" FIELD="DiagnosisNegativeCores" Runat="server" ID="DiagnosisNegativeCores" onchange="setDxBiopsyTotalCores();" />
  
  <euc:EformTextBox style="display: none;" TABLE="ProstateBiopsyPath" RecordId="1" ParentRecordId="1" FIELD="PathNumCores" Runat="server" ID="PathNumCores" onmouseover="setDxBiopsyTotalCores();" onmouseout="setDxBiopsyTotalCores();" /></span></td></tr>
  </table>

<%@ Control Language="c#" Inherits="Caisis.UI.Modules.HeadNeck.Eforms.OralCavityRecurrence" CodeFile="OralCavityRecurrence.ascx.cs" CodeFileBaseClass="Caisis.UI.Core.Classes.BaseEFormControl" %>
...
<a name="OralCavityRecurrence" /><asp:Label ID="ComponentTitle" runat="server" CssClass="controlBoxTitle" Text="Disease Recurrence"></asp:Label><br/>
...
<table width="680" cellpadding="5" cellspacing="0" class="ClinicalEformTable" id="DiseaseRecurTable">     
  ...
  <tr>    
    ...
    <td align="left" style="white-space: nowrap;" >
        <euc:EformDropDownList Width="100px" RecordId="2" TABLE="Status" FIELD="StatusProbability"  ID="StatusProbability_2" Runat="server" ShowEmptyListItem="true" >
            ...
            <asp:ListItem Value="SurgeryCTRT" Text="SurgeryCTRT" ></asp:ListItem>
  ...
  <tr>    
    ...
    <td align="left" style="white-space: nowrap;" >
        <euc:EformTextBox Width="1px" style="display: none;" TABLE="NoTable" FIELD="StatusLocation_3"  ID="StatusLocation_3" Runat="server" />

        <div style="display: none; " runat="server" id="StatusLocationLDIV_3">
            <euc:EformExtendedCheckBoxList TABLE="NoTable" FIELD="StatusLocationL_3"  ID="StatusLocationL_3" Runat="server" RepeatDirection="Horizontal" RepeatColumns="5" RepeatLayout="Flow" >
                ...
                <asp:ListItem Text="L7" Value="L7"></asp:ListItem>
                <asp:ListItem Text="Unknown" Value="Unknown"></asp:ListItem>       
            </euc:EformExtendedCheckBoxList>
        </div>
        <div style="display: none; " runat="server" id="StatusLocationRDIV_3">
            <euc:EformExtendedCheckBoxList TABLE="NoTable" FIELD="StatusLocationR_3"  ID="StatusLocationR_3" Runat="server" RepeatDirection="Horizontal" RepeatColumns="5" RepeatLayout="Flow" >
                <asp:ListItem Text="R1" Value="R1"></asp:ListItem>
                <asp:ListItem Text="R2" Value="R2"></asp:ListItem>
                <asp:ListItem Text="R3" Value="R3"></asp:ListItem>
                <asp:ListItem Text="R4" Value="R4"></asp:ListItem>
                <asp:ListItem Text="R5" Value="R5"></asp:ListItem>
                <asp:ListItem Text="R6" Value="R6"></asp:ListItem>
                <asp:ListItem Text="R7" Value="R7"></asp:ListItem>
                <asp:ListItem Text="Unknown" Value="Unknown"></asp:ListItem>       
            </euc:EformExtendedCheckBoxList>
        </div>            
  ...
  <tr>    
    ...
    <td align="left" style="white-space: nowrap;" colspan="2" valign="top">
        <euc:EformExtendedCheckBoxList DropDown="true" DropDownTextWidth="260px" style="white-space: nowrap;" RecordId="4" TABLE="Status" FIELD="StatusNotes" ID="StatusNotes_4"  Runat="server" LookupCode="DxTarget,Disease,HeadNeck" RepeatDirection="Horizontal" RepeatColumns="4" RepeatLayout="Table" /></td>
    <%--<td align="left" style="white-space: nowrap;" >
...
<script type="text/javascript">
    ...
    {  
        ...
        var leftSideIdObj = document.getElementById('<%= StatusLocationL_3.ClientID  %>');
        var cblLeftSide = leftSideIdObj.getElementsByTagName('input');
      
        for (var j = 0; j < cblLeftSide.length; j++)
        {
            var cblRefLeft = cblLeftSide[j];
            if (cblRefLeft.type == 'checkbox')
                  $(cblRefLeft).addEvent('click',curry(SetHiddenFieldsNeckR,dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,location,side,probability));
            else if (cblRefLeft.type == 'text')
                  $(cblRefLeft).addEvent('blur',curry(SetHiddenFieldsNeckR,dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,location,side,probability));
        } 

        var rightSideIdObj = document.getElementById('<%= StatusLocationR_3.ClientID  %>');
        var cblRightSide = rightSideIdObj.getElementsByTagName('input'); 
        for (var j = 0; j < cblRightSide.length; j++)
        {
            var cblRefRight = cblRightSide[j];
            if (cblRefRight.type == 'checkbox')
                  $(cblRefRight).addEvent('click',curry(SetHiddenFieldsNeckR,dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,location,side,probability));
            else if (cblRefRight.type == 'text')
                  $(cblRefRight).addEvent('blur',curry(SetHiddenFieldsNeckR,dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,location,side,probability));
        } 
        ...
        SetHiddenFieldsNeckR(dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,location,side,probability)
    }
    ...
    function SetHiddenFieldsNeckR(dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,location,side,probability)
	{
        ...
        var leftSideDiv = document.getElementById('<%= StatusLocationLDIV_3.ClientID  %>');
        var rightSideDiv = document.getElementById('<%= StatusLocationRDIV_3.ClientID  %>');
        
        var leftSideIdObj = document.getElementById('<%= StatusLocationL_3.ClientID  %>');
        var rightSideIdObj = document.getElementById('<%= StatusLocationR_3.ClientID  %>');
        
        if (side.value.toLowerCase() == 'left')
        {
            leftSideDiv.style.display = '';
            rightSideDiv.style.display = 'none';
            ResetCheckBoxList (rightSideIdObj);
            location.value = RetrieveCheckBoxListSelectedValues (leftSideIdObj,"FLOW");
        }
        else if (side.value.toLowerCase() == 'right')
        {
            leftSideDiv.style.display = 'none';
            rightSideDiv.style.display = '';
            ResetCheckBoxList (leftSideIdObj);
            location.value = RetrieveCheckBoxListSelectedValues (rightSideIdObj,"FLOW");
        }
        else if (side.value.toLowerCase() == 'bilateral')
        {
            leftSideDiv.style.display = '';
            rightSideDiv.style.display = '';
            location.value = RetrieveCheckBoxListSelectedValues (leftSideIdObj,"FLOW");
            location.value = location.value + "," + RetrieveCheckBoxListSelectedValues (rightSideIdObj,"FLOW");
        }
        else
        {
            leftSideDiv.style.display = 'none';
	    ...
	    if (dateText.value != '' || location.value != ''  || side.value != ''  || probability.value != '' )
	    {
	        ...
	        if (side.value != '')
	           statusNotesHidden.value =  statusNotesHidden.value + "Side: " + side.value + " ||| ";
	        if (location.value != '')
	           statusNotesHidden.value =  statusNotesHidden.value + "Location: Level " + location.value + " ||| ";
	    }
	    else
	    {
	        statusHidden.value = '';
    ...
    {  
        $(dateText).addEvent('blur',curry(SetHiddenFieldsDistantMet,dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,probability));
        $(probability).addEvent('blur',curry(SetHiddenFieldsDistantMet,dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,probability));
    }

   
    /* ]]> */
    
    function SetHiddenFieldsDistantMet(dateText,date,statusHidden,statusNotesHidden,diseaseHidden,qualityHidden,probability)
	{   
